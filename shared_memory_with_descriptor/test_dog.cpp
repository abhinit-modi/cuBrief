...
extern uchar* DoG_detector_cuda
(Mat img, int **k_x, int **k_y, 
int *n, float th_c, float th_r,
int levels, float sigma);

/* Global variable to store key points */
...
Mat test_cuda_dog(Mat img, bool print)
{
    int *kp_x_h, *kp_y_h, kp_h = 0;

    uchar* h_descriptor = DoG_detector_cuda(img, &kp_x_h, &kp_y_h, &kp_h, 7.0, 12.0, 6, 1.0);
    ...
    Mat desc_1(kp_h, 32, CV_8UC1, h_descriptor);
    
    return desc_1;
}

Mat processUsingCuda(std::string input_file, std::string output_file)
{
    ...
    uchar* h_descriptor = DoG_detector_cuda(input_float, &kp_x_h, &kp_y_h, &kp_h, 7.0, 12.0, 6, 1.0);
    Mat desc_1(kp_h, 32, CV_8UC1, h_descriptor);
    drawKeyPoints(input, kp_x_h, kp_y_h, kp_h, output_file);

    return desc_1;
}

Mat match(Mat img_1, Mat img_2, Mat descriptors_1, Mat descriptors_2, std::vector<KeyPoint> kp_1, std::vector<KeyPoint> kp_2, std::string output){
    ...
    matcher->knnMatch(GpuMat(descriptors_1), GpuMat(descriptors_2), matches, 2);  // Find two nearest matches    
    ...
    drawMatches( img_1, kp_1, img_2, kp_2,
               good_matches, img_matches, Scalar(255, 0, 0), Scalar(0, 0, 255));
...
void test_matching(std::string input_file, std::string output_file){
    
    Mat desc_1 = processUsingCuda(input_file, "kp_1.jpg");
    std::vector<cv::KeyPoint> kp_1;
    for(int i=0;i<kp_h;i++) {
        cv::KeyPoint kp(kp_x_h[i], kp_y_h[i], 1.0);
        kp_1.push_back(kp);
    }
    
    Mat desc_2 = processUsingCuda(output_file, "kp_2.jpg");
    ...
    cv::Mat img_1 = cv::imread(input_file, CV_LOAD_IMAGE_GRAYSCALE);
    cv::Mat t_img_2 = cv::imread(output_file, CV_LOAD_IMAGE_GRAYSCALE);
    Mat img_2;
    cv::resize(t_img_2, img_2, Size(), 1, 1);

    match(img_1, img_2, desc_1, desc_2, kp_1, kp_2, "result.jpg");   
}
...
int main(int argc, char ** argv) 
{
    ...
    Mat desc1 = test_cuda_dog(input_float, false);
    ...
    for (int set = 0; set < sets; set++)
    {
        ...
        for (int s = 0; s < n_scales; s++)
        {
            ...
            Mat desc2 = test_cuda_dog(input_float, false);
